$(document).ready(function () {
  ...
  <td><a href='/api/display_manager_picture?managerid=${item.id}&managername=${item.firstname} ${item.lastname}&picture=${item.photograph}'><img src="/${item.photograph}" width="30"></a></ td>
  
  <td><a href='/api/managerbyid?managerid=${item.id}'><button style="-webkit-box-shadow: 0px 8px 25px -7px rgba(0, 0, 0, 0.96);
  -moz-box-shadow: 0px 8px 25px -7px rgba(0, 0, 0, 0.96);
  box-shadow: 0px 8px 25px -7px rgba(0, 0, 0, 0.96);
" type="button" class="btn btn-primary buttonstyle"><b>Update/Delete</b></button>
    </a></td>`
      })
